                                                  ...
                                                  callback:(RCTImageLoaderCompletionBlock)callback
{
  return [self loadImageWithURLRequest:imageURLRequest
                                 ...
                                 scale:1
                               clipped:YES
                            resizeMode:RCTResizeModeFitXY //默认填充
                         progressBlock:nil
                                   ...
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSString *fetchDate))completionHandler
{
  ...
  RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    ...
    if ([schemes containsObject:request.URL.scheme.lowercaseString]) {
      [QRCTImageStoreManager cacheImageData:data forKey:request.URL.absoluteString];
    }
    // QAdd End///////
                                           ...
                                           completionBlock:(RCTImageLoaderCompletionBlock)completionBlock
{
  ...
  // 先判断本地是否有缓存，有缓存返回缓存数据、
  [QRCTImageStoreManager retrieveImageDataForKey:imageURLRequest.URL.absoluteString
                                        complete:^(NSData *imageData, QRImageCacheType cacheType) {
                                          __typeof(self) strongSelf = weakSelf;
                                          if (cancelled || !strongSelf) {
                                            return ;
                                          }
                                          ...
                                          if ((imageData != nil) && (imageData.length > 0)) {
                                            completionHandler(nil, imageData, YES, nil);
                                          } else {
                                            cancelLoad = [strongSelf _loadImageOrDataWithURLRequest:imageURLRequest
                                                                                         size:size
                                                                                        scale:scale
                                                                                   resizeMode:resizeMode
                                                                                progressBlock:progressHandler
