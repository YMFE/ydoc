...
- (instancetype)initWithBridge:(RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    RCTAssert(bridge, @"bridges is a required parameter");
    _bridge = bridge;
...
static void RCTUpdatePlaceholder(QRCTTextField *self)
{
...
- (void)textFieldDidChange
{
  ...
  if (_maxLength && self.text.length > _maxLength.unsignedLongValue) {
    NSRange textRange = [self.text rangeOfComposedCharacterSequencesForRange:NSMakeRange(0, _maxLength.unsignedIntegerValue)];
    self.text = [self.text substringWithRange:textRange];
  }
...
- (NSInteger)cursorPosition {
  ...
  if (!(selectedRange.empty) && self.selectionAffinity == UITextStorageDirectionForward) {
