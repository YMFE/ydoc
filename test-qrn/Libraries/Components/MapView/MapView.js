...
const MapView = React.createClass({
  ...
  propTypes: {
     ...
     * 显示当期路径信息
     */
    routeInfo: React.PropTypes.shape({
    	/**
    	 * 起点经纬度
    	 */
    	routeStartPosition: React.PropTypes.shape({
    	 ...
    	 * 终点经纬度
    	 */
    	routeEndPosition: React.PropTypes.shape({
    		latitude: React.PropTypes.number,
      		longitude: React.PropTypes.number,
    	}),
    	/**
    	 * 路径类型，步行/驾车
    	 */
    	routeType: React.PropTypes.string,
    	/**
    	 * 路径宽度
    	 */
    	 ...
    	 * 是否开启路径功能
    	 */
    	routeEnabled: React.PropTypes.bool,
    }),
    ...
    annotations: React.PropTypes.arrayOf(React.PropTypes.shape({
      ...
      /**
       * 避开从当前subview中取数据造成数组越界,AnnotationImage
       */
      QWHImage: React.PropTypes.source,

      /**
       * 避开从当前subview中取数据造成数组越界，左边的图片
       */
      QWHLeftCallOutImage: React.PropTypes.source,

      /**
       * 避开从当前subview中取数据造成数组越界，右边的图片
       */
      QWHRightCallOutImage: React.PropTypes.source,
  ...
  render: function() {
    ...
    if (this.props.onUserLocationChange) {
      var onLocationChange = (event: Event) => {
        if (event.nativeEvent.locationCoordinate) {
          this.props.onUserLocationChange &&
            this.props.onUserLocationChange(event.nativeEvent.locationCoordinate);
        }
      };
    }
