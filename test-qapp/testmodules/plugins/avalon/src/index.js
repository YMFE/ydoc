...
(function() {
    ...
    var plugins = {
        loader: function(builtin) {
            window.define = (builtin && innerRequire) ? innerRequire.define : otherDefine
            window.require = (builtin && innerRequire) ? innerRequire : otherRequire
        },
    ...
    function notifySubscribers(list) { //通知依赖于这个访问器的订阅者更新自身
        ...
        if (new Date() - beginTime > 444) {
            removeID = setTimeout(removeSubscribers, 1);
        } else {
...
/*============================================
 ...
 309行增加: parseJSON: JSON.parse
 1256行修改: 增加延迟，使 removeSubscribers 非同线程执行
 3131行增加: duplexBinding.TEXTAREA = duplexBinding.INPUT
...
(function() {
    ...
    var PREFIX = 'qapp-',
        AVALON_ID = 0,
        AVALON_TAG = 'ms-controller',
        INIT_STATUS = 'unused';

    var _addWidget = QApp.addWidget;
    ...
    QApp.addWidget = function (name, adapter, isEvent) {
        var eventName = isEvent && (_.isString(isEvent) ? isEvent : 'tap');
        _addWidget(name, adapter, isEvent);
        avalon.ui['qapp-' + name] = function (element, options, vmodels) {
            var view, widget, timer,
                i = -1;
            options = {
                args: {}
            };
            while (!vmodels[++i].getView) {
            }
            view = vmodels[i].getView();
            timer = _.delay(function () {
                _.each(element.dataset, function (key, value) {
                    if (!key.indexOf(name)) {
                        var attrName = key.substring(name.length).replace(/\w/i, function (letter) {
                            return letter.toLowerCase();
                        });
                        if (!attrName.indexOf('arg')) {
                            options.args[
                ...
                // options.vm = vmodels[0];
                if (view) {
                    if (eventName) {
                        eventName = options.eventType || eventName;
                        bindFunc = function () {
                            widget = adapter(element, options, view, vmodels[0]);
                        };
                        _.addEvent(element, eventName, bindFunc);
                        view.on('destroy', function () {
                            _.removeEvent(element, eventName, bindFunc);
                        });
                    } else {
                        widget = adapter(element, options, view, vmodels[0]);
                    }
    ...
    QApp.ready(function () {
        _.createStyle('[ms-controller] {visibility:hidden;}')
    });

    QApp.addPlugin(['avalon', 'engine.avalon'], {}, function (view, options) {
        var avalonId = PREFIX + (AVALON_ID++),
            vm, vms = {},
            _mergeParam = view.mergeParam;

        if (_.isFunction(options)) {
            vm = avalon.define(avalonId, function (vm) {
                vm.view_status = INIT_STATUS;
                vm.getView = function () {
                    return view;
                };
                options.call(view, vm);
            });
        } else {
            _.each(options, function (id, factory) {
                if (_.isFunction(factory)) {
                    vms[id] = avalon.define(avalonId + '-' + id, function (vm) {
                        vm.getView = function () {
                            return view;
                        };
                        factory.call(view, vm);
                    });
                    vms[id].view_status = INIT_STATUS;
                }
            });
        }

        view.getVM = function (key) {
            return key ? vms[key] : vm;
        };

        view.mergeParam = function (param) {
            _mergeParam.call(view, param);
            if (vm) {
                assign(vm, param);
            } else if (vms) {
                _.each(vms, function (key, vmodel) {
                    assign(vmodel, param);
                });
            }
            return view;
        ...
        view.on('loadStart', function () {
            if (_.isFunction(options)) {
                _.attr(view.root.querySelector('[' + AVALON_TAG + ']') || view.root, AVALON_TAG, avalonId);
            } else {
                _.each(options, function (id, factory) {
                    if (_.isFunction(factory)) {
                        _.attr(me.root.querySelector(
                            '[' + AVALON_TAG + '=' + id + ']'
                        ), AVALON_TAG, avalonId + '-' + id);
                    }
                });
            }
        });
        ...
        view.on('rendered', function () {
            avalon.scan(view.root);
            if (view.extra.vmcache && vmcache[view.name]) {
                if (vm) {
                    assign(vm, vmcache[view.name]);
                } else if (vms) {
                    _.each(vms, function (key, vmodel) {
                        assign(vmodel, vmcache[view.name][key]);
                    });
                }
            }
            view.mergeParam(view.param);
        });

        view.on('destroy', function () {
            if (view.extra.vmcache) {
                vmcache[view.name] = serialize(vm ? vm : vms);
            }
            if (vm) {
                vm.$unwatch();
                delete avalon.vmodels[vm.$id]; // 不能 = null 否则会出错
            } else if (vms) {
                _.each(vms, function (key, vmodel) {
                    vmodel.$unwatch();
                    delete avalon.vmodels[vmodel.$id];
