...
var Set = require('./pollyfills/Set')
    ...
    return this._transforms.map(transform => {
      ...
      for (var key in transform) {
        ...
        if (value instanceof Animated) {
          ...
          result += key + '(' + (value.join ? value.join(',') : value) + ')'; // 不是数组
        }
      }
      return result;
    }).join(' ');
  ...
  __getAnimatedValue(): Array<Object> {
    return this._transforms.map(transform => {
      ...
      for (var key in transform) {
        ...
        if ( typeof(value) === 'number' && (key === 'translateX' || key === 'translateY' || key === 'translateZ' || key === 'perspective')) {
          value = parseFloat((value / 100).toFixed(2)) + 'rem';
...
class AnimatedStyle extends AnimatedWithChildren {
  ...
  constructor(style: any) {
    ...
    if (style.transform) {
      style = {
        ...style,
        webkitTransform: new AnimatedTransform(style.transform),
...
class AnimatedProps extends Animated {
  ...
  constructor(
    ...
    if (props.style) {
      props = {
        ...props,
        style: new AnimatedStyle(StyleSheet.normalize(props.style)),
      };
    }
    this._props = props;
...
function createAnimatedComponent(Component: any): any {
  ...
  class AnimatedComponent extends React.Component {
    ...
    componentWillMount() {
      // 一切的根源在于不支持 style={[style1, style2]}
      //this.props.style = StyleSheet.arrayLikeToObject(this.props.style)
...
module.exports = {
  ...
  /**
   * @class Value
   * @description 表示数值的类，通常使用 new Animated.Value(0) 来初始化。
   */
  Value: AnimatedValue,
  /**
