...
export default class ListCore extends ComponentCore {
     ...
     */
    refreshStartIndexByOffsetY(offsetY) {
        offsetY = offsetY - window.screen.height / 5;
    ...
    setItemPositionData(item, attr) {
        ...
        if (item._type === 'groupTitle') {
            ...
            this.positionMap[item.key] && Object.assign(this.positionMap[item.key], attr);
        }
    }
    ...
    renderDataSource(ds, refreshAll = false) {
        ds.forEach((ditem, i)=> {
            ...
            if (this.infinite) {
                ...
                //不定高的情况,如果前一个item具有bottom,设置_translateY为前一个元素的bottom
    ...
    getVisibleList(offsetY = this.offsetY,
        ...
        if (this.infinite) {
            ...
            return dataSource.slice(startIndex, endIndex).map(item=> {
                //将item的原始数据和position信息结合
                return {
                    ...item,
    ...
    updateTranslateY(notResolvedItemIndex, dataSource = this.dataSource) {
        let notResolvedItem = dataSource[notResolvedItemIndex];
    ...
    resolveItem(key, height, dataSource = this.dataSource) {
        ...
        let targetItem = this.dataSource[targetIndex];
        let _translateY;
    ...
    getTotalHeight(dataSource = this.dataSource) {
        return dataSource.reduce((acc, item)=> {
            const itemPosData = this.getItemPositionData(item);
            acc += itemPosData._resolved ? itemPosData.height : 0;
            return acc;
        }, 0);
    }
